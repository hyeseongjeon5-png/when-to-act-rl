...
왜 필요한가:
    검사가 [맞음]을 내는 것은 문제가 없다는 뜻일 수도, 검사가 아무것도 못 찾고
    있다는 뜻일 수도 있다. **아무것도 못 찾는 것과 문제가 없는 것은 다르다.**
    그래서 통과를 보고 믿지 않고, 일부러 고장을 넣어 잡히는지를 확인한다.
...
def test_process_roots() -> None:
    """탐지가 '기능하는지'를 본다 — 0을 반환하는 것과 없는 것을 구분해야 한다.
