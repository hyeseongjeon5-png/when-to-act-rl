...
| 산출물 | λ-성능 지도, "학습이 규칙을 이기는 임계 비용" 표, 졸업논문 원고(`졸업논문_초안v3.docx`) |
...
- [x] **4주차 — 보강 실험 스프린트 (2026-08-29 ~ 08-31)** — 이슈 6건 모두 닫힘
  - [x] **공정성 검증** ([#11](../../issues/11)) — λ=0에서 예산을 3.3배로 늘리고 탐험·신경망을
    바꿔도 규칙에 닿지 못함. "비용 때문이 아니라 학습이 덜 된 탓 아닌가"라는 반론에 답한다
  - [x] **붕괴 문턱 정밀 측정** ([#12](../../issues/12)) — 붕괴가 **두 단계**임을 발견.
    행동 횟수는 그대로인데 목표 도달률이 먼저 무너지는 구간(λ 0.0075~0.01)이 있다
  - [x] **인과 실험** ([#13](../../issues/13)) — 예산의 앞 절반만 비용 없이 학습시키면
    도달률이 0% → 66%로 올라간다(λ=0.05, 표준 DQN). **붕괴는 최적해가 아니라 탐험 실패다**
  - [x] **기준선 감사** (목록 밖) — 비교 상대인 규칙의 계수를 다시 골랐더니 LunarLander의
    규칙이 126점 올랐고 그 환경의 λ\*가 바뀌었다.
    **학습만 의심하고 규칙은 의심하지 않으면 결론이 부풀려진다**
  - [x] **LunarLander λ\* 정밀화** ([#15](../../issues/15)) — λ 격자를 8 → 12개로 촘촘히 해도
    λ\*가 그대로였다. 성긴 격자의 우연이 아니라는 확인
  - [x] **MinAtar Freeway 확장** ([#14](../../issues/14)) ★ — 세 계열 모두 λ\*=0.
    이 환경은 **규칙 품질로는 가운데, 보상 조밀도로는 희소 쪽 끝**이라 두 후보 설명을 가른다.
    **λ\*는 규칙 품질이 아니라 보상 조밀도를 따라간다**
  - [x] **공정성 검증 확장** ([#16](../../issues/16)) — 세 환경 모두에서 "예산을 더 주면
    이겼을 것"이라는 반론이 막혔다. LunarLander는 예산 5배에 **오히려 나빠졌다**
  - [ ] 로드맵 보강 3건 (붕괴 문턱의 예산 의존 · 비용 부과 방식 민감도 · Lazy 기본 정책) — 진행 중
- [ ] 논문 (`paper/` → `졸업논문_초안v3.docx`) — Ⅰ~Ⅴ장 · **그림 8점 · 표 15점 · 약 34쪽**
- [ ] 발표 준비
...
|---|---|---|---|---|---|
| MountainCar-v0 | **0** | **0** | **0** | 10 | 세 계열 모두 λ=0에서도 규칙을 넘어서지 못한다 |
| LunarLander-v3 | **1** | **0** | **2** | 10 | 1개 계열은 λ=0에서도 못 이긴다 — 방법에 따라 크게 갈린다 |
| MinAtar_Freeway-v1 | **0** | **0** | **0** | 10 | 세 계열 모두 λ=0에서도 규칙을 넘어서지 못한다 |

### MountainCar-v0 — 보상이 희소하고 좋은 규칙이 있다

| 계열 | λ=0 | λ=0.001 | λ=0.003 | λ=0.0075 | λ=0.02 | λ=0.05 | λ=0.2 | λ=0.66 |
|---|---|---|---|---|---|---|---|---|
| **표준 DQN** | -160 | -172 | -165 | -197 | -200 | -200 | -200 | -200 |
| **TempoRL** | -141 | -145 | -149 | -183 | -200 | -200 | -200 | -200 |
| **Lazy-MDP** | -128 | -152 | -144 | -168 | -159 | -187 | -200 | -200 |
...
|---|---|---|---|---|---|---|---|
| **표준 DQN** | 189 | 189 | 90 | -33 | -150 | -134 | -210 |
| **TempoRL** | 159 | 119 | 59 | -28 | -202 | -153 | -286 |
| **Lazy-MDP** | 210 | 213 | 103 | 5 | -61 | -157 | -175 |
...
### MinAtar Freeway — 규칙이 쓸 만하지만 이길 여지가 있다

| 계열 | λ=0 | λ=0.005 | λ=0.01 | λ=0.02 | λ=0.04 | λ=0.06 |
|---|---|---|---|---|---|---|
| **표준 DQN** | 7 | 2 | 0 | 0 | -0 | 0 |
| **TempoRL** | 9 | 2 | 0 | -0 | -0 | 0 |
| **Lazy-MDP** | 14 | 13 | 11 | 8 | 2 | -1 |
| cautious 규칙(기준) | 17 | 15 | 14 | 11 | 5 | -1 |
| 최강 규칙 | 17 | 15 | 14 | 11 | 7 | 2 |
| 무행동 | 0 | 0 | 0 | 0 | 0 | 0 |

조건당 환경 300,000스텝 · 시드 10개 · 값은 비용 반영 총보상 r′의 IQM (λ 격자 6개 중 일부만 표시)

![MinAtar_Freeway-v1 λ-성능 지도](results/figures/MinAtar_Freeway-v1_lambda_map_cost_return.png)
...
# 그 감시가 '실제로 잡는지' 일부러 고장 내어 시험 (12종)
python -m src.monitor.watchdog_selftest
...
- **감시 장치도 시험한다.** 검사가 통과하는 것을 보고 안심하지 않고, 일부러 고장을 넣어
  잡는 것을 확인한 뒤에야 믿는다. 아무것도 못 찾는 것과 문제가 없는 것은 다르기 때문이다

## 이어서 작업하려면

`docs/02_실험-설계.md`(설계)와 `docs/03_로드맵.md`(일정)에 무엇을 왜 그렇게 정했는지가
적혀 있다. 실험을 다시 돌리는 절차와 명령어는 아래 '재현하기'에 있다.
