...
- [ ] **4주차 — 보강 실험 스프린트 (2026-08-29 진행 중)**
  - [x] **공정성 검증** — λ=0에서 예산을 3.3배로 늘려도 규칙에 닿지 못함을 확인.
    "비용 때문이 아니라 학습이 덜 된 탓 아닌가"라는 반론에 답한다
  - [ ] 붕괴 문턱 정밀 측정 — λ 0.001~0.0075 구간을 채워 무행동 붕괴가 시작되는 비용을 특정
  - [ ] 인과 실험 — 비용을 켜는 시점만 바꿔 붕괴가 최적해인지 탐험 실패인지 판정
  - [ ] LunarLander λ\* 부근 정밀화 (λ 1.2·1.6·1.8·2.5 추가)
  - [ ] MinAtar Freeway 확장 (다섯 게임 기준선을 재고 선정)
  - [ ] 공정성 검증을 LunarLander·MinAtar로 확장
...
|---|---|---|---|---|---|
| MountainCar-v0 | **0** | **0** | **0** | 10 | λ=0에서도 규칙을 넘어서지 못한다 |
| LunarLander-v3 | **2** | **2** | **2** | 10 | 넓은 비용 구간에서 학습이 이긴다 |

### MountainCar-v0 — 보상이 희소하고 좋은 규칙이 있다

| 계열 | λ=0 | λ=0.01 | λ=0.02 | λ=0.03 | λ=0.05 | λ=0.1 | λ=0.2 | λ=0.35 | λ=0.66 |
|---|---|---|---|---|---|---|---|---|---|
| **표준 DQN** | -160 | -197 | -200 | -200 | -200 | -200 | -200 | -200 | -200 |
| **TempoRL** | -141 | -193 | -200 | -200 | -200 | -200 | -200 | -200 | -200 |
| **Lazy-MDP** | -128 | -156 | -159 | -167 | -187 | -192 | -200 | -200 | -200 |
| pump 규칙(기준) | -119 | -121 | -122 | -123 | -125 | -131 | -143 | -161 | -198 |
| 최강 규칙 | -119 | -121 | -122 | -123 | -125 | -131 | -143 | -161 | -198 |
| 무행동 | -200 | -200 | -200 | -200 | -200 | -200 | -200 | -200 | -200 |

조건당 환경 300,000스텝 · 시드 10개 · 값은 비용 반영 총보상 r′의 IQM (λ 격자 9개 중 일부만 표시)

![MountainCar-v0 λ-성능 지도](results/figures/MountainCar-v0_lambda_map_cost_return.png)

### LunarLander-v3 — 보상이 조밀하고 규칙이 약하다

| 계열 | λ=0 | λ=0.1 | λ=0.3 | λ=0.6 | λ=1 | λ=1.37 | λ=2 | λ=3 |
|---|---|---|---|---|---|---|---|---|
| **표준 DQN** | 189 | 189 | 141 | 90 | -13 | -61 | -134 | -210 |
| **TempoRL** | 159 | 119 | 81 | 59 | -34 | -52 | -153 | -286 |
| **Lazy-MDP** | 210 | 213 | 147 | 103 | 37 | -47 | -157 | -175 |
| threshold 규칙(기준) | 36 | 24 | -1 | -37 | -86 | -131 | -207 | -328 |
| 최강 규칙 | 36 | 24 | -1 | -37 | -86 | -131 | -131 | -131 |
| 무행동 | -131 | -131 | -131 | -131 | -131 | -131 | -131 | -131 |

조건당 환경 200,000스텝 · 시드 10개 · 값은 비용 반영 총보상 r′의 IQM (λ 격자 8개 중 일부만 표시)
...
- **MountainCar (희소 보상 + 강한 규칙)**: 에피소드 보상의 1%에 해당하는 λ=0.01만으로도
  ...
  멀고 무작위 탐험으로는 닿을 수 없기 때문이다(측정 도달률 **0.0%**). 학습은 "아무것도 하지
  않는" 정책으로 얼어붙는다. → **규칙을 쓰라.**
- **LunarLander (조밀 보상 + 약한 규칙)**: 지정 기준선만 보면 격자 끝까지 학습이 이긴다.
  그러나 λ>1.366부터 최강 규칙이 **무행동**으로 바뀌고, λ=2.0에서 세 계열 모두 그 아래로
  내려간다 — 이것이 실질적 λ\*다. → **학습을 쓰라.**
...
"규칙에 진 것은 비용 때문이 아니라 학습이 덜 된 탓 아닌가?" — 이 반론을 먼저 확인했다.
