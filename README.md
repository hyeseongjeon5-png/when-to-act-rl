...
- [ ] **4주차 — 보강 실험 스프린트 (2026-08-29 진행 중)**
  ...
  - [x] **기준선 감사** (목록 밖) — 비교 상대인 규칙의 계수를 다시 골라 보았더니
    LunarLander의 규칙이 141점 올랐고, 그 결과 그 환경의 λ\*가 바뀌었다.
    **학습만 의심하고 규칙은 의심하지 않으면 결론이 부풀려진다**
  - [ ] LunarLander λ\* 부근 정밀화 ([#15](../../issues/15)) — 진행 중
  - [ ] MinAtar Freeway 확장 ([#14](../../issues/14)) — 고정 규칙 완료, 학습 계열 대기
  - [ ] 공정성 검증 확장 ([#16](../../issues/16)) — LunarLander·MinAtar
- [ ] 논문 (`paper/` → `졸업논문_초안v1.docx`) — Ⅰ~Ⅴ장 초안·그림 5점·표 11점 완성(약 26쪽),
  최종 수치 갱신 대기. 자체 심사표로 그림·조판을 다시 손봄 → `docs/06_자체심사.md`
- [ ] 발표 준비
...
| MinAtar_Freeway-v1 | **0** | **0** | **0** | 1 | 세 계열 모두 λ=0에서도 규칙을 넘어서지 못한다 |
...
|---|---|---|---|---|---|---|
| **표준 DQN** | 7 | 2 | 0 | -0 | -0 | -0 |
| **TempoRL** | 10 | 2 | -1 | -1 | 0 | 0 |
| **Lazy-MDP** | 15 | 12 | 12 | 10 | 3 | -0 |
| cautious 규칙(기준) | 17 | 15 | 14 | 11 | 5 | -1 |
| 최강 규칙 | 17 | 15 | 14 | 11 | 7 | 2 |
| 무행동 | 0 | 0 | 0 | 0 | 0 | 0 |

조건당 환경 300,000스텝 · 시드 2개 · 값은 비용 반영 총보상 r′의 IQM (λ 격자 6개 중 일부만 표시)
